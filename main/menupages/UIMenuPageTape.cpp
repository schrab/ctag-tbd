    ...
    namespace CTRL {
        ...
        void UIMenuPageTape::redrawFileList() {
            ...
            for (int i = 0; i < visible; i++) {
                ...
                Display::DrawString(0, 10 + i * 9, fileNames[idx], Display::FONT_5X7);
            }
            int cy = 10 + (cursor - scrollOffset) * 9;
            Display::InvertRect(0, cy, 128, 8);
            if (fileCount > 6) Display::DrawScrollbar(126, 10, 54, fileCount, cursor);
            Display::Flush();
        }

        void UIMenuPageTape::redrawRecord() {
            Display::Clear();
            Display::DrawString(0, 10, "START REC", Display::FONT_5X7);
            Display::DrawString(0, 19, "CANCEL", Display::FONT_5X7);
            Display::InvertRect(0, 10 + cursor * 9, 128, 8);
            Display::Flush();
        }
    }
}
